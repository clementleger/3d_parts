...
/* Y Rod holder */
ROD_X_OFFSET_FROM_BEAM = MAIN_BEAM_HEIGHT - CHASSIS_JOINT_THICKNESS;
...
MAIN_BEAM_LENGTH = X_ROD_LENGTH - 2 * ROD_Y_OFFSET_FROM_BEAM;

/* Table */
TABLE_WIDTH = Y_ROD_LENGTH - 2 * Y_ROD_HOLDER_THICKNESS + 2 * CHASSIS_JOINT_THICKNESS;
TABLE_HEIGHT = MAIN_BEAM_LENGTH - CHASSIS_JOINT_THICKNESS * 2 - MAIN_BEAM_WIDTH * 2;
...
NEMA_WIDTH = 42.3;
NEMA_LENGTH = 48;
NEMA_HOLDER_THICKNESS = 5;

module nema_holder()
{
    cube([NEMA_WIDTH + REAL_BEAM_HOLDER_WIDTH, NEMA_LENGTH,CHASSIS_JOINT_THICKNESS]);
    cube([NEMA_WIDTH, NEMA_HOLDER_THICKNESS, NEMA_WIDTH]);
}
