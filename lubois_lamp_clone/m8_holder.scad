...
module side_holder(side = 1) {
    difference() {
        ...
        translate([HOLDER_HEIGHT/2 , HOLE_DIAMETER + side * SIDE_THICKNESS, 0]) cylinder(d = HOLE_DIAMETER, h = THICKNESS);
    }
}

side_holder();
translate([0, SPACE_BETWEEN_SIDES + WOOD_WIDTH, 0]) side_holder(-1);
