...
inner_diameter_in_millimeter = 10.50;
...
outer_diameter_in_millimeter = 17.0; 
...
bushing_length_in_millimeter = 25;
...
LIFT_PROFILE_WIDTH = 28;
LIFT_THICKNESS = outer_diameter_in_millimeter/2;
LIFT_HOLE_DIAMETER = 6;
ROUNDED_SIZE = LIFT_PROFILE_WIDTH / 2;
NUT_SIZE = 11.5;

//translate([-1.5, (LIFT_PROFILE_WIDTH / 4 * 3) / 2 + outer_diameter_in_millimeter / 2 - 1.5, bushing_length_in_millimeter/2])
difference() {
    roundedcube([outer_diameter_in_millimeter/2, LIFT_PROFILE_WIDTH / 4 * 3, bushing_length_in_millimeter], false, 1.75, "ymax");
    
    translate([0, outer_diameter_in_millimeter/2 + NUT_SIZE/2 - 1, bushing_length_in_millimeter / 2])
    rotate([0, 90, 0])
    union() {
        cylinder(d = LIFT_HOLE_DIAMETER, h = LIFT_THICKNESS);
         translate([0, 0, - outer_diameter_in_millimeter / 4 + 2]) #cylinder(d = NUT_SIZE, h = LIFT_THICKNESS, center= true, $fn=6);
    }
    cylinder(d = outer_diameter_in_millimeter, h = bushing_length_in_millimeter);
//   
//   translate([LIFT_THICKNESS - 2, LIFT_PROFILE_WIDTH/2 - (LIFT_PROFILE_WIDTH / 4 * 3)/2 + 2,0])rotate([0, 90, 0]) cylinder(d = NUT_SIZE, h = LIFT_THICKNESS, center= true, $fn=6);
}
